...
class post_excel extends CI_Controller{
    ...
    function print_excel($post_sawt_type,$post_org_id,$post_quarter,$post_account_number){	
        ...
        # GET QUARTER DATE    
        $result_quarter = $this->get_report->get_quarter($post_quarter);
        ...
        # MAIN HEADER
        $worksheet->write(0,0,"BIR FILE CREATION REPORT ".$row->label_name." ".$row->label_year,$headerFormat);
        for($i=1;$i<13;$i++) {
        ...
        $worksheet->setColumn(8,8,30);
        $worksheet->setColumn(9,9,30);
        $worksheet->setColumn(10,10,30);
        $worksheet->setColumn(10,11,15);
        $worksheet->setColumn(10,12,15);
        
        # HEADER
        $worksheet->write(2,0,"TIN",$headerFormat);
        $worksheet->write(2,1,"BIR REG NAME",$headerFormat);
        $worksheet->write(2,2,"ACCOUNT NUMBER",$headerFormat); 
        $worksheet->write(2,3,"ACCOUNT NAME",$headerFormat);
        $worksheet->write(2,4,"RECEIPT NUMBER",$headerFormat);
        $worksheet->write(2,5,"RECEIPT DATE",$headerFormat); 
        $worksheet->write(2,6,"GL DATE",$headerFormat);   
        $worksheet->write(2,7,"GL DATE SAWT",$headerFormat);
        $worksheet->write(2,8,"AMOUNT",$headerFormat);
        $worksheet->write(2,9,"TAX BASE",$headerFormat);
        $worksheet->write(2,10,"EWT RATE",$headerFormat);
        $worksheet->write(2,11,"ATC CODE",$headerFormat);    
        $worksheet->write(2,12,"ORG ID",$headerFormat);         
        
        $data = $this->get_report->get_post($post_sawt_type,$post_org_id,$post_quarter,$post_account_number);
        ...
        foreach($data as $row){  
            ...
            $worksheet->write($ctr,1,$row->BIR_REG_NAME,$row_left); 
            ...
            $worksheet->write($ctr,6,date('Y-m-d',strtotime($row->GL_DATE)),$row_center);
            $worksheet->write($ctr,7,date('Y-m-d',strtotime($row->GL_DATE_SAWT)),$row_center);
            $worksheet->write($ctr,8,$row->AMOUNT,$row_right_number);
            $worksheet->write($ctr,9,$row->TAX_BASE,$row_right_number);
            $ewt_rate = (($row->AMOUNT/$row->TAX_BASE)*100); 
            $worksheet->write($ctr,10,$ewt_rate,$row_right_number);
            $worksheet->write($ctr,11,$row->ATC_CODE,$row_left);  
        ...
        // $worksheet->write($ctr,5,$total_amount,$headerFormat2);             
        // $worksheet->write($ctr,9,$total_tax_base,$headerFormat2);             
