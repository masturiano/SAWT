...
class cancelled_excel extends CI_Controller{
    ...
    function print_excel($oracle_user_name,$org_id){	
        ...
        # FILENAME
        $filename = "Cancelled.xls";
        
        # SEND FILENAME
        $workbook->send($filename);
        
        # ADD WORK SHEET OR WORK SHEET LABEL
        $worksheet = &$workbook->addWorksheet('Cancelled');
        ...
        # MAIN HEADER
        $worksheet->write(0,0,"CANCELLED SAWT REPORT",$headerFormat);
        for($i=1;$i<13;$i++) {
        ...
        $worksheet->write(2,12,"CANCELLED DATE",$headerFormat);
        
        # DATABASE
        $this->load->model('get_report'); 
        
        $data = $this->get_report->get_cancelled($oracle_user_name,$org_id);
        ...
        foreach($data as $row){  
            ...
            if(date('Y-m-d',strtotime($row->CANCELLED_DATE)) == '1970-01-01'){
                $worksheet->write($ctr,12,'',$row_center);        
            }
            else{
                $worksheet->write($ctr,12,date('Y-m-d',strtotime($row->CANCELLED_DATE)),$row_center);    
            }
           
            $ctr++;
        }  
               
        $data_total = $this->get_report->get_cancelled_total($oracle_user_name,$org_id);
