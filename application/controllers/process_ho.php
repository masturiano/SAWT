...
class Process_ho extends CI_Controller {
    ...
    public function encode(){  
        ...
        # MODULE NAME
        $data['module_name']  = "Encode Process";   
    ...
    public function encode_per_company(){  
        ...
        <script type="text/javascript"> 
            ...
            function cancel(value){
                ...
                $.ajax({
                    url: "<?php echo base_url('process_ho/cancel_export');?>",
                    type: "POST",
                    data: "post_receipt_id="+value,
                    success: function(data){
                        $('#div_cancel_confirm .modal-body').html(data);
                        $('#div_cancel_confirm').modal('show');
                        $('#cancelling_confirm').click( function (e) {
                            e.stopImmediatePropagation();
                            $.ajax({
                                url: "<?php echo base_url('process_ho/cancel_export_update');?>",
                                type: "POST",
                                data: "post_receipt_id="+value,
                                success: function(){
                                    bootbox.alert("Receipt number successfully cancelled!", function() {  
                                        $('#div_cancel_confirm').modal('hide'); 
                                        document.location.reload();
                                    });       
                                }         
                            });     
                    ...
                    success: function(data){   
                        ...
                        if(value.length > 1){
                            ...
                            $('#saving').click( function (e) {
                                ...
                                if(display != '0.50' && display != '1.00' && display != '2.00' && display != '5.00'){
                                    bootbox.alert("Please check tax base, tax percent must be 0.50 or 1.00 or 2.00 or 5.00!", function() {
                                        $('#txt_tax_base').css("border","red solid 1px");
                                    });    
                                    return false;
                                }
                                ...
                                if($('#txt_period_from').val() != 'NULL' && $('#txt_period_to').val() != 'NULL'){
                                    if($('#txt_period_from').val().length == 0){
                                        bootbox.alert("Please input Period From!", function() {
                                            $('#txt_period_from').css("border","red solid 1px");
                                        });    
                                        return false;
                                    }
                                    else{
                                        $('#txt_period_from').css("border","gray solid 1px");    
                                    }      
                                    if($('#txt_period_to').val().length == 0){
                                        bootbox.alert("Please input Period To!", function() {
                                            $('#txt_period_to').css("border","red solid 1px");
                                        });    
                                        return false;
                                    }
                                    else{
                                        $('#txt_period_to').css("border","gray solid 1px");    
                                    } 
                                    
                                    var dateStart = $('#txt_period_from').val();
                                    var dateEnd = $('#txt_period_to').val();
                                ...
                                $('#div_save_confirm .modal-body').html('Are you sure you want to update receipt numberrr '+$('#txt_receipt_no').val()+'?');
            ...
            <button id="btn_cancel" type="button" class="btn btn-default" disabled="disabled">Cancel</button>         
    ...
    public function cancel_export(){  
        ...
        echo "Are you sure you want to cancel receipt number ".$row->RECEIPT_NUMBER."?";    
    } 
    
    public function cancel_export_update(){  
        # MODEL
        $this->load->model('get_process_ho');
        ...
        $this->get_process_ho->cancel_receipt_id($this->input->post('post_receipt_id'));
        echo "Export has been edited cancelled receipt number insert into cancel table";
    }
    ...
    public function view_export(){  
        ...
        <form id="encode_form" method="POST">
            ...
            <table id="table_edit" border="0" class="table table-condensed table-striped">    
                ...
                <tr>
                    <td class="table_label"><b>TRANSMITTAL NUMBER</b></td>
                    <td class="table_colon"><b>:</b></td>
                    <td class="table_data">
                        
                        <?php 
                        if(strlen($row->TRANSMITTAL_NUMBER) > 0){
                            echo $row->TRANSMITTAL_NUMBER;    
                        }
                        else{
                            echo "";    
                        }
    ...
    public function encode_export(){  
        ...
        <form id="encode_form" method="POST">
            ...
            <table id="table_edit" border="0" class="table table-condensed table-striped">    
                ...
                <?php if(stripos($row->NAME,"2307") !== false){?>
                <tr>
                    <td class="table_label"><b>Period</b></td>
                    <td class="table_colon"><b>:</b></td>
                    <td class="table_data">
                        <input class="datepicker form-control" name="txt_period_from" id="txt_period_from" 
                        data-date-format="yyyy-mm-dd"
                        style="height:30px; width: 191px; float: left;" value="" placeholder="From"> 
                        <p style="float: left;"> &nbsp - &nbsp </p>
                        <input class="datepicker form-control" name="txt_period_to" id="txt_period_to" 
                        data-date-format="yyyy-mm-dd"
                        style="height:30px; width: 191px; float: left;" value="" placeholder="To"> 
                    </td>
                </tr>
                <?php }else{
                ?>
                <tr style="border-bottom: transparent;">   
                    <td class="table_label" style="border-bottom: transparent;">
                        <input type="hidden" name="txt_period_from" id="txt_period_from" value="NULL">
                        <input type="hidden" name="txt_period_to" id="txt_period_to" value="NULL">
                    </td>
                </tr>   
                <?php    
                }?>
    ...
    public function encode_export_update(){  
        ...
        if($this->input->post('txt_period_from') == 'NULL' && $this->input->post('txt_period_to') == 'NULL'){
            $edit_export = array(
            ...
            $edit_export = array(
                ...
                "ATC_CODE" => $this->input->post('cmb_atc_code'),
                "TAX_BASE" => $this->input->post('txt_tax_base'),
                "PERIOD_FROM_DATE" => $this->input->post('txt_period_from'),
                "PERIOD_TO_DATE" => $this->input->post('txt_period_to')
            );     
        }
    ...
    public function transmit_export_details(){  
        ...
        echo "Are you sure you want to transmit selected receipt number?";
    ...
    public function transmit_export_update(){  
        ...
        $result_transmittal_number = $this->get_process_ho->get_transmittal_number();

        if ($result_transmittal_number->num_rows() > 0){
           $row_trans_no = $result_transmittal_number->row();
        }
        else{
           $row_trans_no = "0"; 
        }
        ...
        $edit_export = array(
            ...
            "TRANSMITTAL_NUMBER" => 'SAWT-'.str_pad($row_trans_no->last_transmittal_no, 18, '0', STR_PAD_LEFT)
        );
    ...
    {   
        ...
        echo "window.open('".base_url()."transmittal_ho_pdf/index/".str_replace(',','-',$receipt_id)."');";                  
    }
    ...
    public function receive(){  
        ...
        # MODULE NAME
        $data['module_name']  = "Receive Process";   
        ...
        $data['get_all_company'] = $this->get_process_ho->get_all_company();
        $data['get_all_stores'] = $this->get_process_ho->get_store_details();  
            ...
            data-keep-selection="true">
                <thead>
                    <tr class="clickable-row"> 
                        ...
                        <th data-column-id="tax_base">TAX BASE</th>
                        <th data-column-id="gl_date_sawt">SAWT GL DATE</th>
                        <th data-column-id="transmittal_number">TRANSMITTAL NUMBER</th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                    ...
                    <tr>
                        ...
                        <td id="<?php echo $row->CASH_RECEIPT_ID; ?>"><?php echo $row->TAX_BASE; ?></td> 
                        <td id="<?php echo $row->CASH_RECEIPT_ID; ?>"><?php echo date('Y-m-d',strtotime($row->GL_DATE_SAWT)); ?></td>
                        <td id="<?php echo $row->CASH_RECEIPT_ID; ?>"><?php echo $row->TRANSMITTAL_NUMBER; ?></td> 
                    </tr>
                    <?php
                        }
                    ?>
    ...
    public function received_edit(){  
        ...
        # MODULE NAME
        $data['module_name']  = "Received Edit Process";   
    ...
    public function post(){  
        ...
        # MODULE NAME
        $data['module_name']  = "BIR File Creation Process";   
    ...
    public function post_per_company(){  
        ...
        # MODULE NAME
        $data['module_name']  = "BIR File Creation";   
        ...
        <script type="text/javascript">
            ...
            function post(company,quarter){
                ...
                $('#posting_confirm').click( function (e) {
                e.stopImmediatePropagation();    
                    $.ajax({
    ...
    public function create_textfile(){
        ...
        $result_company = $this->get_process_ho->get_company($this->input->post('post_company_id')); 
        if ($result_company->num_rows() > 0){
           $row_company = $result_company->row();
        }   
        
        $result_quarter = $this->get_process_ho->get_quarter($this->input->post('post_quarter_id')); 
        if ($result_quarter->num_rows() > 0){
           $row_quarter = $result_quarter->row();
        }     
        
        $label_number_for_filename = str_replace('/','',$row_quarter->label_number);
        $file_extension = ".dat";
        $file_name = $row_company->glstid."0000".$label_number_for_filename."1702q".$file_extension; 
                  
        $file_content_header = "HSAWT,H1702Q,{$row_company->glstid},0000,\"{$row_company->comp_name}\",\"\",\"\",\"\",{$row_quarter->label_number},{$row_company->rdo_num}";
        
        $file_content = "";
                            
        $file_content .= trim($file_content_header);
        $file_content .= "\r\n";
        foreach($result_textfile as $row){     
            $file_content .= trim($row->FIRST_COLUMN).","; # 1. DEFAULT
            $file_content .= trim($row->SECOND_COLUMN).","; # 2. DEFAULT 
            $file_content .= trim($row->THIRD_COLUMN).","; # 3. SERIES
            $file_content .= trim($row->TIN).","; # 4. TIN NUMBER
            $file_content .= trim($row->FIFTH_COLUMN).","; # 5. DEFAULT
            $file_content .= "\"".trim($row->BIR_REG_NAME)."\","; # 6. BIR REG NAME
            $file_content .= ",,,"; # EXTRA THREE COMMA
            $file_content .= trim($row_quarter->label_number).","; # 7. QUARTER NUMBER
            $file_content .= ","; # EXTRA ONE COMMA
            $file_content .= trim($row->ATC_CODE).","; # 8. ATC CODE
            $file_content .= trim($row->TAX_PERCENT).","; # 9. TAX PERCENT   
            $file_content .= trim($row->TAX_BASE).","; # 10. TAX PERCENT
            $file_content .= trim($row->AMOUNT); # 11. ORACLE AMOUNT
            $file_content .= "\r\n";  
            
            $total_tax_base += $row->TAX_BASE;
            $total_amount += $row->AMOUNT;
        }
        $display_tax_base = $total_tax_base;
        $display_amount = $total_amount;
        
        $file_content_footer = "CSAWT,C1702Q,{$row_company->glstid},0000,{$row_quarter->label_number},{$display_tax_base},{$total_amount}";
        $file_content .= trim($file_content_footer);
