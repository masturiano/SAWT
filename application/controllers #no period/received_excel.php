...
class received_excel extends CI_Controller{
    ...
    function print_excel($oracle_user_name,$org_id,$date_from,$date_to){	
        ...
        # FILENAME
        $filename = "Received.xls";
        
        # SEND FILENAME
        $workbook->send($filename);
        
        # ADD WORK SHEET OR WORK SHEET LABEL
        $worksheet = &$workbook->addWorksheet('Received');
        ...
        # MAIN HEADER
        $worksheet->write(0,0,"RECEIVED SAWT REPORT RECEIPT DATE FROM ".date('F d, Y',strtotime($date_from))." TO ".date('F d, Y',strtotime($date_to)),$headerFormat);
        for($i=1;$i<14;$i++) {
        ...
        $worksheet->setColumn(10,12,20);
        $worksheet->setColumn(10,13,15);
        ...
        $worksheet->write(2,12,"RECEIVED BY",$headerFormat);
        $worksheet->write(2,13,"RECEIVED DATE",$headerFormat);
        
        # DATABASE
        $this->load->model('get_report'); 
        
        $data = $this->get_report->get_received($oracle_user_name,$org_id,$date_from,$date_to);
        ...
        foreach($data as $row){  
            ...
            $worksheet->write($ctr,12,$row->RECEIVED_BY,$row_left);
            $worksheet->write($ctr,13,date('Y-m-d',strtotime($row->RECEIVED_DATE)),$row_center);
           
            $ctr++;
        }  
               
        $data_total = $this->get_report->get_received_total($oracle_user_name,$org_id,$date_from,$date_to);
